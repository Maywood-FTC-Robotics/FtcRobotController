...
public class PIDController{
    ...
    public void setSetpoint(double setpoint) {
        ...
        if (m_continuous) {
            ...
            m_positionError = MathUtils.inputModulus(m_setpoint - m_measurement, -errorBound, errorBound);
        } else {
    ...
    public double calculate(double measurement) {
        ...
        if (m_lastTimeStamp == 0) m_lastTimeStamp = currentTimeStamp;
        m_period = currentTimeStamp - m_lastTimeStamp;
        m_lastTimeStamp = currentTimeStamp;

        if (Math.abs(m_period) > 1E-6) {
            m_velocityError = (m_positionError - m_prevError) / m_period;
        } else {
            m_velocityError = 0;
        }
